...
const Bubble = ({
  ...
  if (colorClass === "green") {
    ...
    if (isCorrect === true)
      baseColor = "bg-green-500 border-green-500"; // Acertou
    else if (isCorrect === false)
      baseColor = "bg-red-500 border-red-500"; // Errou (marcado errado)
    else if (isCorrect === "missed")
      baseColor =
        "bg-yellow-400 border-yellow-400 text-white"; // Era a certa mas não marcou
    ...
    <div className="fixed inset-0 z-10000 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full animate-scale-in border border-gray-100">
        ...
        <div className="flex gap-3">
          <button
            onClick={onCancel}
            className="flex-1 py-2.5 bg-gray-100 text-gray-700 rounded-xl font-bold hover:bg-gray-200 transition text-sm"
          >
            {cancelText}
          </button>
          <button
            onClick={onConfirm}
            className={`flex-1 py-2.5 text-white rounded-xl font-bold shadow-md transition text-sm ${
              isDestructive
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh] animate-scale-in">
        ...
        <div className="p-6 border-t flex gap-3 bg-gray-50 rounded-b-2xl">
          <button
            onClick={onClose}
            className="flex-1 py-3 text-gray-600 font-bold hover:bg-gray-200 rounded-xl"
          >
            Cancelar
          </button>
          <button
        ...
        <div className="p-6 border-b flex justify-between items-start bg-gray-50 rounded-t-2xl">
          <div>
            ...
            <div className="flex gap-4 text-sm text-gray-600 mt-1">
              <span>
                Matrícula: <b>{result.matricula}</b>
              </span>
              <span>
                Universidade: <b>{result.university || "N/A"}</b>
              </span>
            </div>
          </div>
                    ...
                    <div className="space-y-2">
                      {["a", "b", "c", "d", "e"].map((item, subIdx) => {
                        const correctVal = Array.isArray(correctAnswer)
                          ? correctAnswer[subIdx]
                          : null;
                        const studentVal = Array.isArray(studentAnswer)
                          ? studentAnswer[subIdx]
                          : null;
                      ...
                      {CHOICES_MULT.map((c) => {
                        ...
                        return (
                          <Bubble
                            ...
                            isCorrect={state}
                            isMarked={
                              c === studentAnswer || c === correctAnswer
                            }
                          />
                        );
                      })}
...
const StudentExam = ({ examData, onFinishExam, onBack }) => {
  ...
  useEffect(() => {
    if (examData && examData.questions) {
      const initialAnswers = examData.questions.map((q) =>
        q.type === "true_false" ? [null, null, null, null, null] : null
      );
  ...
  const handleMark = (questionIdx, value, subIdx = null) => {
    ...
    setAnswers((prev) => {
      const newAns = [...prev];
      if (subIdx !== null) {
        const currentArr = Array.isArray(newAns[questionIdx])
          ? [...newAns[questionIdx]]
          : [null, null, null, null, null];
        currentArr[subIdx] = value;
        newAns[questionIdx] = currentArr;
      } else {
        newAns[questionIdx] = value;
      }
      return newAns;
    });
    setErrorMsg("");
  ...
  const handlePreSubmit = () => {
    if (!name.trim() || !matricula.trim() || !university.trim())
      return setErrorMsg("Preencha Nome, Matrícula e Universidade.");
    setShowConfirm(true);
  };
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center">
          ...
          <button
            ...
            className="mt-4 w-full py-3 bg-gray-800 text-white rounded-xl hover:bg-gray-900 transition"
          >
            Nova Prova
          </button>
        </div>
    ...
    <div className="min-h-screen bg-gray-50 p-4 max-w-3xl mx-auto">
      <div className="bg-white rounded-2xl shadow-xl border border-gray-200 p-6 space-y-6">
            ...
            {q.type === "true_false" ? (
              <div className="pl-4 space-y-2 mt-2">
                {["a", "b", "c", "d", "e"].map((sub, sIdx) => (
                  <div
                    key={sub}
                    className="flex gap-4 items-center justify-between sm:justify-start"
                  >
                    ...
                    <div className="flex gap-2">
                      {CHOICES_BOOL.map((c) => (
                        <Bubble
                          ...
                          label={c}
                          size="small"
                          isMarked={answers[idx] && answers[idx][sIdx] === c}
                          onMark={() => handleMark(idx, c, sIdx)}
                          colorClass="green"
                        />
                      ))}
                    </div>
              ...
              <div className="flex gap-2 mt-2 justify-center sm:justify-start">
                {CHOICES_MULT.map((c) => (
                  <Bubble
                    key={c}
                    label={c}
                    isMarked={answers[idx] === c}
                    onMark={() => handleMark(idx, c)}
                  />
                ))}
              </div>
            )}
        ...
        <button
          ...
          className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-md transition"
        >
...
const ExamDetail = ({ exam, onUpdateExam, onBack, onSimulate, userId }) => {
  ...
  const handleKeyChange = (qIdx, val, sIdx = null) => {
    setTempKey((prev) => {
      const newKey = [...prev];
      if (sIdx !== null) {
        const arr = Array.isArray(newKey[qIdx])
          ? [...newKey[qIdx]]
          : [null, null, null, null, null];
        arr[sIdx] = val;
        newKey[qIdx] = arr;
  ...
  const filteredResults = results.filter((r) => {
    const nameMatch = r.studentId
      .toLowerCase()
      .includes(filterName.toLowerCase());
    const matMatch = r.matricula
      .toLowerCase()
      .includes(filterMatricula.toLowerCase());
    const uniMatch = (r.university || "")
      .toLowerCase()
      .includes(filterUniversity.toLowerCase());
    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
        <h1 className="text-3xl font-extrabold text-blue-800">
          {fullExamData.subject}
        </h1>
        <p className="text-gray-500 text-xs font-mono bg-gray-100 px-2 py-1 rounded inline-block mt-2">
          ID: {fullExamData.id}
        </p>
      </div>
        ...
        <div className="lg:col-span-1 flex flex-col h-full">
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 flex-1 flex flex-col">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                onClick={() =>
                  isEditingKey ? handleSaveKey() : setIsEditingKey(true)
                }
                disabled={isSaving}
              ...
              >
                {isSaving ? "..." : isEditingKey ? "Salvar" : "Editar"}
              </button>
            </div>
            {msg && (
              <div className="mb-3 p-2 text-xs rounded border bg-blue-50 text-blue-700 border-blue-100">
                {msg}
              </div>
            )}
                            ...
                            <div className="flex flex-col gap-1">
                              {CHOICES_BOOL.map((o) => (
                                <button
                                  ...
                                  onClick={() => handleKeyChange(idx, o, si)}
                      ...
                      <div className="flex gap-1 justify-center">
                        {CHOICES_MULT.map((o) => (
                          <button
                            key={o}
                            disabled={!isEditingKey}
                            onClick={() => handleKeyChange(idx, o)}
                            className={`w-8 h-8 text-xs rounded font-bold ${
                              currentAnswer === o
        ...
        <div className="lg:col-span-2 bg-white p-5 rounded-2xl shadow-sm border border-gray-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
            <input
              ...
              className="p-2 border rounded-lg text-sm bg-gray-50 text-black"
            />
            <input
      ...
      {showQR && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
          <div className="bg-white p-8 rounded-2xl text-center relative max-w-sm w-full">
            <button
            ...
            <p className="text-xs text-gray-500 mb-2">
              Peça para os alunos escanearem ou acesse:
            </p>
            <input
              value={examUrl}
              readOnly
              className="w-full p-2 border rounded text-xs text-center bg-gray-50"
  ...
  return (
    <>
      <div className="space-y-8 animate-fade-in-up">
        <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border gap-4">
          ...
          <button
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-bold shadow-lg transition transform hover:-translate-y-0.5"
          >
            + Nova Prova
          </button>
        </div>
        <div className="grid grid-cols-1 gap-4">
          {exams.length === 0 ? (
            ...
            exams.map((exam) => (
              <div
                key={exam.id}
                className="bg-white p-5 rounded-2xl shadow-sm border flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:shadow-md transition"
              >
                <div>
                ...
                <div className="flex gap-2 w-full sm:w-auto">
                  <button
                    ...
                    className="flex-1 sm:flex-none bg-blue-50 hover:bg-blue-100 text-blue-600 px-4 py-2 rounded-lg font-bold text-sm transition"
                  >
                    Gerenciar
                  </button>
                  <button
                    onClick={() => setExamToDelete(exam)}
                    className="flex-1 sm:flex-none bg-red-50 hover:bg-red-100 text-red-600 px-4 py-2 rounded-lg font-bold text-sm transition"
                  >
                    Excluir
                  </button>
                </div>
      ...
      <ConfirmModal
        ...
        message={`Tem certeza que deseja excluir "${examToDelete?.subject}"? Todos os resultados serão perdidos.`}
        confirmText="Sim, excluir"
        isDestructive={true}
        onConfirm={() => handleDelete(examToDelete.id)}
...
export default function Page() {
  return (
    <Suspense
      fallback={<div className="p-10 text-center">Carregando aplicação...</div>}
    >
      <Content />
      <style jsx global>{`
